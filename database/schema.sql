PRAGMA foreign_keys = ON;

-- INDEX: idx_fact_inv_fecha
CREATE INDEX idx_fact_inv_fecha ON fact_inventario(Fecha);

-- INDEX: idx_fact_inv_tienda
CREATE INDEX idx_fact_inv_tienda ON fact_inventario(Id_Tienda);

-- INDEX: idx_fact_ventas_fecha
CREATE INDEX idx_fact_ventas_fecha ON fact_ventas(Fecha);

-- INDEX: idx_fact_ventas_tienda
CREATE INDEX idx_fact_ventas_tienda ON fact_ventas(Id_Tienda);

-- TABLE: Inventario
CREATE TABLE "Inventario" (
"Id_Inventario" INTEGER,
  "Fecha" TIMESTAMP,
  "Id_Tienda" INTEGER,
  "Categoria" TEXT,
  "Promocion" INTEGER,
  "Precio" REAL,
  "Descuento_pct" REAL,
  "Lead_Time_Dias" INTEGER,
  "Stock_Minimo" INTEGER,
  "Stock_Objetivo" INTEGER,
  "Stock_Disponible" INTEGER,
  "Pedido_Pendiente" INTEGER,
  "Rotura_Stock" INTEGER
);

-- TABLE: Supermercado
CREATE TABLE "Supermercado" (
"Id_Tienda" INTEGER,
  "Nombre_Supermercado" TEXT,
  "Ciudad" TEXT
);

-- TABLE: Ventas
CREATE TABLE "Ventas" (
"Id_Venta" INTEGER,
  "Fecha" TIMESTAMP,
  "Nombre_Supermercado" TEXT,
  "Ciudad" TEXT,
  "Categoria" TEXT,
  "Cantidad_Vendida" INTEGER,
  "Monto_Total" REAL,
  "Id_Tienda" INTEGER
);

-- TABLE: dim_tienda
CREATE TABLE dim_tienda (
  Id_Tienda INTEGER PRIMARY KEY,
  Nombre_Supermercado TEXT NOT NULL,
  Ciudad TEXT NOT NULL,
  Region TEXT NOT NULL
);

-- TABLE: fact_inventario
CREATE TABLE fact_inventario (
  Id_Inventario INTEGER PRIMARY KEY,
  Fecha TEXT NOT NULL,
  Id_Tienda INTEGER NOT NULL,
  Categoria TEXT NOT NULL,
  Promocion INTEGER,
  Descuento_pct REAL,
  Lead_Time_Dias INTEGER,
  Stock_Minimo REAL,
  Stock_Objetivo REAL,
  Precio REAL,
  Cantidad_Vendida REAL,
  Stock_Disponible REAL,
  Pedido_Pendiente REAL,
  Rotura_Stock INTEGER,
  FOREIGN KEY (Id_Tienda) REFERENCES dim_tienda(Id_Tienda)
);

-- TABLE: fact_ventas
CREATE TABLE fact_ventas (
  Id_Venta INTEGER PRIMARY KEY,
  Fecha TEXT NOT NULL,
  Id_Tienda INTEGER NOT NULL,
  Categoria TEXT NOT NULL,
  Cantidad_Vendida REAL NOT NULL,
  Monto_Total REAL NOT NULL,
  FOREIGN KEY (Id_Tienda) REFERENCES dim_tienda(Id_Tienda)
);

-- TABLE: stg_inventario
CREATE TABLE stg_inventario (
  Id_Inventario INTEGER,
  Fecha TEXT,
  Id_Tienda INTEGER,
  Nombre_Supermercado TEXT,
  Ciudad TEXT,
  Categoria TEXT,
  Promocion INTEGER,
  Descuento_pct REAL,
  Lead_Time_Dias INTEGER,
  Stock_Minimo REAL,
  Stock_Objetivo REAL,
  Precio REAL,
  Cantidad_Vendida REAL,
  Stock_Disponible REAL,
  Pedido_Pendiente REAL,
  Rotura_Stock INTEGER
);

-- TABLE: stg_supermercado
CREATE TABLE stg_supermercado (
  Id_Tienda INTEGER,
  Nombre_Supermercado TEXT,
  Ciudad TEXT,
  Region TEXT
);

-- TABLE: stg_ventas
CREATE TABLE stg_ventas (
  Id_Venta INTEGER,
  Fecha TEXT,
  Nombre_Supermercado TEXT,
  Ciudad TEXT,
  Categoria TEXT,
  Cantidad_Vendida REAL,
  Monto_Total REAL,
  Id_Tienda INTEGER
);
